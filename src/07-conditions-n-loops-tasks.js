...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.width < rect2.left - rect1.left) { return false; }
  if (rect2.width < rect1.left - rect2.left) { return false; }
  if (rect1.height < rect2.top - rect1.top) { return false; }
  if (rect2.height < rect1.top - rect2.top) { return false; }
  return true;
}
...
function isInsideCircle(circle, point) {
  const normX = point.x - circle.center.x;
  const normY = point.y - circle.center.y;
  return (normX * normX + normY * normY) < circle.radius * circle.radius;
}
...
function evaluateTicTacToePosition(position) {
  const rows = position.slice();
  const cols = [0, 1, 2].map((col) => position.map((row) => row[col]));
  const diagonal1 = position.map((row, i) => row[i]);
  const diagonal2 = position.map((row, i) => row[2 - i]);
  ...
  const winner = checks
    ...
    .find((check) => check === 'XXX' || check === '000');
